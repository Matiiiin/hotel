...
//        }
        ...
        for ($i =0 ; $i<$book_data['number_of_rooms'] ; $i++){
        $room_number = RoomNumber::where('rooms_id' , $room->id)->where('status' , 'در دسترس')->inRandomOrder()->first();
        $room_number->status = 'رزرو شده';
        $room_number->save();
        }
        Session::forget('book_date');
...
//        dd($data);
    ...
    public function DownloadInvoice($id){
        ...
        $invoice_name = 'invoice_'. $editData->code;
    ...
    public function addBooking(Request $request)
    {
        ...
        $previousely_booked = Booking::where('rooms_id' , $request->room_id)
              ->whereBetween('check_in' , [date('Y-m-d',strtotime($request->check_in)), date('Y-m-d',strtotime($request->check_out))])
//            ->where('check_in' , '>=' ,$request->check_in)
//            ->where('check_out' , $request->check_out)
            ->get();
        if (!count($previousely_booked)>0){
            $data = array();
            $data['number_of_rooms'] = $request->number_of_rooms;
            $data['persion'] = $request->persion;
            $data['check_in'] = date('Y-m-d',strtotime($request->check_in));
    ...
    public function AdminBookingCreate()
    {
        $roomtype = RoomType::all();
        $users = User::where('status' , 'active')->get();
        return view('backend.booking.add_booking',compact(['roomtype' , 'users']));
    }
    ...
    public function AdminBookingStore(Request $request)
    {
        ...
        $data = new Booking();
        $data->rooms_id = $room->id;
        $data->user_id = $book_data['user_id'];
        $data->check_in = date('Y-m-d',strtotime($book_data['check_in']));
        $data->check_out = date('Y-m-d',strtotime($book_data['check_out']));
        $data->number_of_rooms = $book_data['number_of_rooms'];
        $data->total_night = $total_nights;
